...
export default function BottomNav() {
  const navItems = [
    { name: "Home", to: "/", icon: HomeIcon, exact: true },        // ← go to "/" and use end
  ...
  return (
    <nav className="fixed bottom-0 left-0 w-full bg-white border-t shadow-inner h-14 border-gray-200 p-2 flex justify-around items-center">
      {navItems.map((item) => {
        ...
        return (
          <NavLink
            key={item.name}
            to={item.to}
            end={item.exact} // ← only true for Home
            className={({ isActive }) =>
